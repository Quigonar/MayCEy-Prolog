...
asignar_pista(Matricula,Pista,Hora,Y):-Y==1, Pista=='P3', not(vuelosP3(_,Hora)), asserta(vuelosP3(Matricula,Hora)), write('Su pista asignada es la P3, por favor disminuya su velocidad para que logre aterrizar a las '), write(Hora), nl, nl, sexta_entrada(L),!.
...
% ANALIZA LA ENTRADA DIVIDIENDO EN UNA LISTA Y DESPUES VERIFICA SI LA ORACION ESTA BIEN ESCRITA
analizar_entrada(X):- my_read(X), oracion(X,[]),!.

% REVISA LAS ENTRADAS DE FORMA ORDENADA
...
% LLAMA LA INICION DEL PROGRAMA
maycey():-
        primer_entrada(X).

% LIMPIA LA BASE DE DATOS DE LA PISTA
limpiar_pistas():-
...
% Escribir funciones en consola para mostrar los vuelos registrados.
% vuelosP1(X,Y).
% vuelosP21(X,Y).
% vuelosP22(X,Y).
% vuelosP3(X,Y).